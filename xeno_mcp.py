"""
xeno_mcp.py — Xeno Executor MCP Server v6 - Final Fix

Install:  pip install fastmcp requests pyperclip
Run:      python xeno_mcp.py
"""
...
ENDPOINT = f"{BASE_URL}/o"
TIMEOUT  = 5

mcp = FastMCP("Xeno Executor")
...
def _execute(pid, script: str) -> bool:
    headers = {
        "Content-Type": "text/plain",
        "Clients": json.dumps([str(pid)])
    }
    r = requests.post(ENDPOINT, headers=headers, data=script.encode(), timeout=TIMEOUT)
...
def _resolver_lua() -> str:
    """Lua resolvePath() — NO typeof, pure type() only."""
    return r"""
...
def _write_result_lua(outfile: str) -> str:
    """Injects writeResult(text) helper bound to a specific unique outfile."""
    return f"""
...
def _execute_and_read(pid: int, lua_code: str, wait: float = 2.0) -> str:
    """
    Execute lua_code (must call writeResult(...)) then
    read back via separate script with unique marker validation.
    Retries up to 4 times if marker doesn't match.
    """
    outfile = _unique_file()
    marker  = _unique_marker()

    # Build prefix — outdir + writeResult bound to THIS outfile
    ...
    # Read-back script — uses its own local variables, no placeholders
    read_lua = (
    ...
    # Retry loop — only accept clipboard that starts with OUR marker
...
def check_api_status() -> str:
    ...
    try:
        ...
        return f"✅ Xeno API is UP (HTTP {r.status_code})."
    except requests.ConnectionError:
        return "❌ Xeno API is DOWN."
...
def list_clients() -> str:
    ...
    try:
        ...
        lines = [f"PID {c[0]} | {c[1]} | {c[2]}" for c in clients]
...
def execute_script(lua_code: str, pid: int | None = None) -> str:
    """
    Execute Lua on a client.

    Args:
        lua_code: Lua source to run.
        pid: Target PID (optional, auto-picks first).
    """
    try:
        clients = _get_clients()
        if not clients:
...
def broadcast_script(lua_code: str) -> str:
    ...
    try:
        ...
        results = []
        for c in clients:
            ok = _execute(c[0], lua_code)
            results.append(f"{'✅' if ok else '❌'} PID {c[0]} ({c[1]})")
        return "Broadcast:\n" + "\n".join(results)
    except Exception as e:
        return f"ERROR: {e}"
...
if fn == nil or type(fn) ~= "function" then
    writeResult("ERROR: not a valid function: " .. tostring(err))
    return
end
...
def trace_function_calls(function_names: str, duration: int = 10, pid: int | None = None) -> str:
    ...
    try:
        ...
        for i, fn in enumerate(funcs):
            hook_blocks += f"""
do
    local _ok{i}, _orig{i} = pcall(function()
        return resolvePath("{fn}")
    end)
    if _ok{i} and _orig{i} and type(_orig{i}) == "function" then
        ...
        local _n{i} = function(...)
            ...
            table.insert(__calls, {{
                fn   = "{fn}",
                args = table.concat(ss, ", "),
                t    = tick() - __t0
            }})
            return _o{i}(...)
        end
...
if not (debug and debug.getupvalues) then
    writeResult("ERROR: debug.getupvalues not available on this executor")
    return
end
local visited = {{}}
local lines   = {{
    "=== UPVALUE DUMP: {function_path} ===",
    "Recursive: {rec_str}  MaxDepth: {max_depth}",
    ""
}}
local function dump(f, depth, prefix)
    ...
    if not ok2 or not uvs or not next(uvs) then
        table.insert(lines, prefix .. "[no upvalues]")
        return
    end
    for name, val in pairs(uvs) do
        local vt = type(val)
        ...
        elseif vt == "string" then
            table.insert(lines, prefix .. tostring(name)
                .. ' = "' .. val:sub(1, 80) .. '"')
        else
            table.insert(lines, prefix .. tostring(name)
                .. " = " .. tostring(val) .. " (" .. vt .. ")")
...
if not (debug and debug.getupvalues) then
    writeResult("ERROR: debug.getupvalues not available")
    return
end
local ok2, uvs = pcall(debug.getupvalues, fn)
if not ok2 or not uvs then
    writeResult("ERROR: could not get upvalues")
    return
end
...
for name, val in pairs(uvs) do
    local hit = false
    if stype == "name" then
        hit = string.lower(tostring(name)):find(term, 1, true) ~= nil
    else
        hit = string.lower(tostring(val)):find(term, 1, true) ~= nil
    end
    if hit then
...
out = out .. "Term: '{search_term}'  Mode: {search_type}\\n"
out = out .. "Found: " .. #matches .. "\\n\\n"
for i, m in ipairs(matches) do
    out = out .. i .. ". " .. m.name .. " = "
              .. m.val:sub(1, 100) .. " (" .. m.vt .. ")\\n"
end
writeResult(out)
"""
        return _execute_and_read(target, lua, wait=1.5)
...
if not (debug and debug.setupvalue) then
    writeResult("ERROR: debug.setupvalue not available")
    return
end
local evok, newVal = pcall(function()
    return loadstring("return {new_value_expr}")()
end)
if not evok then
    writeResult("ERROR: invalid value expression")
    return
end
local result = debug.setupvalue(fn, "{upvalue_name}", newVal)
local out = "=== MODIFY UPVALUE ===\\n"
out = out .. "Function: {function_path}\\n"
out = out .. "Upvalue:  {upvalue_name}\\n"
out = out .. "NewValue: " .. tostring(newVal) .. "\\n"
out = out .. "Result:   " .. (result and "SUCCESS" or "FAILED") .. "\\n"
writeResult(out)
"""
...
def inspect_table_deep(table_path: str, max_depth: int = 4, show_metatables: bool = False, pid: int | None = None) -> str:
    """
    Deep recursive inspection of a Lua table (not for Roblox Instances — use
    get_instance_properties for those).

    Args:
        table_path: Dot-path to table e.g. "_G", "game.Players".
...
if type(tbl) ~= "table" then
    writeResult("ERROR: resolved to type='" .. type(tbl)
        .. "' (not a table). For Roblox Instances use get_instance_properties.")
    return
end
local visited = {{}}
local lines   = {{
    "=== DEEP TABLE: {table_path} (depth={max_depth}) ==="
}}
...
def search_in_table(table_path: str, search_key: str = "", search_value: str = "", pid: int | None = None) -> str:
    ...
    Note: works on Lua tables only (_G, module tables) — not on Roblox Instances.
...
if type(tbl) ~= "table" then
    writeResult("ERROR: type='" .. type(tbl)
        .. "' is not a Lua table. Use find_instances for Roblox Instances.")
    return
end
...
local function search(t, path)
    ...
    for k, v in pairs(t) do
        ...
        if km and vm then
            table.insert(matches, {{
                path = kp,
                val  = tostring(v):sub(1, 80),
                vt   = type(v)
            }})
        end
        if type(v) == "table" and #matches < 200 then
            search(v, kp)
        end
    end
end
...
out = out .. "Key='{search_key}'  Value='{search_value}'\\n"
out = out .. "Found: " .. #matches .. "\\n\\n"
for i, m in ipairs(matches) do
    if i > 100 then out = out .. "  ...\\n" break end
    out = out .. i .. ". " .. m.path .. " = " .. m.val
              .. " (" .. m.vt .. ")\\n"
end
writeResult(out)
"""
...
@mcp.tool()
def compare_tables(table_path1: str, table_path2: str, pid: int | None = None) -> str:
    """
    Shallow-compare two Lua tables and report differences.
    Both paths must resolve to actual Lua tables (type == "table").

    Args:
        table_path1: Dot-path to first table  e.g. "_G".
        table_path2: Dot-path to second table e.g. "game.Players".
        pid: Target PID (optional).
    """
    try:
        clients = _get_clients()
        ...
        lua = _resolver_lua() + f"""
local t1, e1 = resolvePath("{table_path1}")
if t1 == nil or type(t1) ~= "table" then
    writeResult("ERROR: first path is not a table: " .. tostring(e1))
    return
end
local t2, e2 = resolvePath("{table_path2}")
if t2 == nil or type(t2) ~= "table" then
    writeResult("ERROR: second path is not a table: " .. tostring(e2))
    return
end
local onlyA, onlyB, diff = {{}}, {{}}, {{}}
local same = 0
for k, v in pairs(t1) do
    if t2[k] == nil then
        table.insert(onlyA, k)
    elseif tostring(t2[k]) ~= tostring(v) then
        table.insert(diff, {{k=k, v1=tostring(v), v2=tostring(t2[k])}})
    else
        same = same + 1
    end
end
for k in pairs(t2) do
    if t1[k] == nil then table.insert(onlyB, k) end
end
local out = "=== TABLE COMPARE ===\\n"
out = out .. "A: {table_path1}\\n"
out = out .. "B: {table_path2}\\n\\n"
out = out .. "Same: " .. same
          .. "  Diff: " .. #diff
          .. "  Only-A: " .. #onlyA
          .. "  Only-B: " .. #onlyB .. "\\n\\n"
if #onlyA > 0 then
    out = out .. "=== ONLY IN A (" .. #onlyA .. ") ===\\n"
    for i, k in ipairs(onlyA) do
        if i > 30 then out = out .. "  ...\\n" break end
        out = out .. "  " .. tostring(k)
                  .. " = " .. tostring(t1[k]) .. "\\n"
    end
    out = out .. "\\n"
...
if #onlyB > 0 then
    out = out .. "=== ONLY IN B (" .. #onlyB .. ") ===\\n"
    for i, k in ipairs(onlyB) do
        if i > 30 then out = out .. "  ...\\n" break end
        out = out .. "  " .. tostring(k)
                  .. " = " .. tostring(t2[k]) .. "\\n"
    end
...
if #diff > 0 then
    out = out .. "=== DIFFERENT VALUES (" .. #diff .. ") ===\\n"
    for i, d in ipairs(diff) do
        if i > 30 then out = out .. "  ...\\n" break end
        out = out .. "  " .. tostring(d.k) .. ":\\n"
        out = out .. "    A = " .. d.v1:sub(1, 60) .. "\\n"
        out = out .. "    B = " .. d.v2:sub(1, 60) .. "\\n"
    end
end
writeResult(out)
"""
...
def compare_instances(instance_path1: str, instance_path2: str, pid: int | None = None) -> str:
    ...
    Use this instead of compare_tables when comparing Instances.
...
if i1 == nil then
    writeResult("ERROR: first path: " .. tostring(e1))
    return
end
local i2, e2 = resolvePath("{instance_path2}")
if i2 == nil then
    writeResult("ERROR: second path: " .. tostring(e2))
    return
end
local props = {{
...
out = out .. "A: {instance_path1}\\n"
out = out .. "B: {instance_path2}\\n\\n"
local same, diff = 0, 0
for _, pn in ipairs(props) do
    ...
    if aok and bok and av ~= nil and bv ~= nil then
        ...
        if as == bs then
            ...
            diff = diff + 1
            out = out .. "  " .. pn .. ":\\n"
            out = out .. "    A = " .. as:sub(1, 60) .. "\\n"
            out = out .. "    B = " .. bs:sub(1, 60) .. "\\n"
        end
    end
end
...
out = out .. "Result: "
          .. (hok and "SUCCESS" or "FAILED: " .. tostring(herr)) .. "\\n"
writeResult(out)
"""
        return _execute_and_read(target, lua, wait=1.5)
    except Exception as e:
...
if n == 0 then
    table.insert(lines, "(no active hooks)")
end
table.insert(lines, "\\nTotal: " .. n)
writeResult(table.concat(lines, "\\n"))
"""
...
setmetatable(proxy, {{
    __index = function(_, key)
        if {log_a} then
            table.insert(accessLog, {{key=tostring(key), t=tick()-t0}})
        end
    ...
    __tostring = function()
        return "MCPProxy[" .. tostring(targetObj) .. "]"
    end
}})
local reg = getgenv and getgenv() or _G
reg.__mcp_proxies = reg.__mcp_proxies or {{}}
reg.__mcp_proxies["{target_path}"] = {{
    proxy     = proxy,
    accessLog = accessLog,
    modLog    = modLog
}}
local out = "=== PROXY CREATED ===\\n"
out = out .. "Target:    {target_path}\\n"
out = out .. "LogReads:  {log_a}\\n"
out = out .. "LogWrites: {log_m}\\n\\n"
out = out .. "Stored at __mcp_proxies['{target_path}']\\n"
out = out .. "Call get_proxy_logs('{target_path}') to view activity."
writeResult(out)
"""
        return _execute_and_read(target, lua, wait=1.5)
    except Exception as e:
...
if not data then
    writeResult("ERROR: no proxy for '{proxy_path}'. "
        .. "Call create_proxy_object first.")
    return
end
local lt  = "{log_type}"
...
if inst == nil then
    if writefile then
        pcall(writefile, "{outfile}", "ERROR: " .. tostring(err))
    end
    return
end
...
if evtStr == "all" then
    for _, e in ipairs({{
        ...
        "DescendantAdded","DescendantRemoving","AncestryChanged"
    }}) do
        table.insert(evtNames, e)
    end
    pcall(function()
        for key, val in pairs(inst) do
            if type(val) == "RBXScriptSignal" then
                table.insert(evtNames, key)
            end
        end
    end)
else
    for name in evtStr:gmatch("[^,]+") do
        table.insert(evtNames, name:match("^%s*(.-)%s*$"))
    end
end
local log   = {{}}
local t0    = tick()
...
out = out .. "Hooked: " .. #conns .. "  |  Fired: " .. #log .. "\\n\\n"
...
local function cap(evType, child)
    table.insert(log, {{
        ...
        class = tostring(child.ClassName),
        t     = tick() - t0
    }})
end
table.insert(conns,
    inst.ChildAdded:Connect(function(c) cap("ChildAdded", c) end))
table.insert(conns,
    inst.ChildRemoved:Connect(function(c) cap("ChildRemoved", c) end))
table.insert(conns,
    inst.DescendantAdded:Connect(function(c) cap("DescendantAdded", c) end))
table.insert(conns,
    inst.DescendantRemoving:Connect(function(c) cap("DescendantRemoving", c) end))
task.wait({duration})
for _, c in ipairs(conns) do pcall(function() c:Disconnect() end) end
local out = "=== LIFECYCLE: {instance_path} ({duration}s) ===\\n"
out = out .. "Total events: " .. #log .. "\\n\\n"
for i, e in ipairs(log) do
    if i > 200 then out = out .. "  ...\\n" break end
    out = out .. string.format("[%.2fs] %s: %s (%s)\\n",
...
string.char = function(...)
    local r = __oldChar(...)
    if #r >= {min_length} and not __seen[r]
        and r:match("[a-zA-Z0-9]") then
        __seen[r] = true
...
for i, s in ipairs(__strings) do
    out = out .. i .. ". " .. s .. "\\n"
end
writeResult(out)
"""
...
#     "xeno": {
#       "command": "python",
#       "args": ["C:/ABSOLUTE/PATH/xeno_mcp.py"]
#     }
#   }
# }